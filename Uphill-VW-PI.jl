        ...
        for j in j2
            ...
            if((p[k,i,j]>1.1 || p[k,i,j]<-0.1)||((p[k,i,j]>1.1 || p[k,i,j]<-0.1)&&(abs(v[j]-v_max)<del)))
                if(b[k,i,j]<0||b[k,i,j]>1)
                    ...
                    J[k,i,j] =(q1[k]*.01*m_f[k,i,j])+(q2[k]*2*u[i]*e[i]*e[i])+(q3[k]*abs(-v[j]+v[i]))+(q5[k]*b[k,i,j])
                end
            else
                b[k,i,j] = 0
                gr[k,i,j] = G[i]
...
for j = 1:1:(n-1)
    ...
    #Acceleration:
    dot_v[j] = c_1*(c_2*T_e[j])-(c_1*k_b*(B[j]))-(c_1*c_3*((V[j]*V[j])))-(c_1*c_8*sin(alpha[j]+c_7))
    #(c_8*sin(alpha[j]+c_7)))
...
ylim(20,42) # Set the y-limits from 0.0 to 1.0
...
xticks(0:500:2500,fontsize = 20) # Set the x axis to a logarithmic scale
xlim(0,2500)
yticks(60:10:90,fontsize = 20) # Set the y-tick range and step size, 0.1 to 0.9 in increments of 0.2
ylim(60,90)
plot(xlin,vlin,"-",linewidth = 4,label="Velocity(Varying-Weights)")
plot(xlin,ref,"_",linewidth = 4,color="r",label="Reference Speed")
#plot(xlin,vlin1,"_",color="b",label = "PI")
plot(xlin,vlin2,"--",color ="black",linewidth = 4,label = "Velocity(PI)")
annotate("Drop In Speed",fontsize = 20,
xy=[1250;72],# Arrow tip
    xytext=[800;72], # Text offset from tip
...
ylim(0.0,1.1)
plot(xlin,plin,"-",linewidth = 4,label="Accelerator(Varying-Weights)")
plot(xlin,blin,"o",linewidth = 4,label="Brake(Varying-Weights)")
plot(xlin,plin2,"--",linewidth = 4,color = "black",label="Accelerator(PI)")
plot(xlin,blin2,"-",linewidth = 4,color = "black",label="Brake(PI)")
xlabel("Distance(m)",fontsize = 20)
...
xlabel("Distance(m)",fontsize = 20)
grid("on")
yticks(1:1:12,fontsize = 16)
ylim(1,13)
plot(xlin,glin,"-",linewidth = 4,label="Gear(Varying-Weights)")
#plot(xlin,glin1,"_",label="PI")
plot(xlin,glin2,"_",linewidth = 4,color = "black",label="Gear(PI)")
ylabel("Gear Position",fontsize = 20)
xlabel("Distance(m)",fontsize = 20)
legend(loc ="lower right",fontsize = 20)
#axis("tight")
fig[:canvas][:draw]() # Update the figure
suptitle("Performance Analyses Of PI Vs Varying-Weights Optimal Control (Fuel Savings = -0.44%)",fontsize = 20)

opt_mf

(561.487 - 1000.67)/(1000.67)
